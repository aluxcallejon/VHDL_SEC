...
k_U3 = 0;
l_U3 = 1;
for cont_1 = 0 : (times*(Nbps)-1)         %Recorremos el numero de simbolos OFDM a transmitir
                                          %Aleatorizamos el vector de salida
OutPutU3(l_U3,(Nbps/s)*(mod(k_U3,s))+floor(k_U3/s)+1)=InputU3(k_U3+(l_U3-1)*Nbps+1);

if(k_U3 == Nbps-1)
    k_U3=0;%Reseteamos cuenta
    l_U3= l_U3 + 1;
else
    k_U3 = k_U3 + 1; %Incrementamos en uno K
end
end
...
%Se creara una matriz de 128 filas. Cada una de ellas contendrá el número complejo que se le pasará al IFFT
  ...
  k_U4=1; %Valores por defecto
  z_U4=1;
  aux = reshape(OutPutU3(z_U4,:), Nbpc,[]).';    %Agrupamos OutputU3 en simbolos de Nbpc
  for contador_U4=0:(95*times+1)                 %Recorremos los datos validos
    ...
    if(k_U4 == 97)
        ...
        k_U4 = k_U4 + 1;      %Aumentamos en uno el valor de k_U4
    end
    simb_fila=aux(k_U4-1,:);                                %Cogemos un vector
    Simb=str2double(strrep(num2str(simb_fila), ' ', ''));   %[1 1 0 ] --> 110 (Ejemplo)
    Abk=find(Cgrey==Simb)-1;                                %Calculamos el incremento de fase
    fase_ant=T(z_U4,k_U4-1);                                %Calculamos la fase anterior
    inc=(2*pi/M)*Abk;                                       %Calculamos el incremento real que hemos dado
    T(z_U4,k_U4)	=	mod(fase_ant+inc,2*pi);             %Calculamos la tetha
    Vport(z_U4,k_U4)=A*exp(1*1i*T(z_U4,k_U4));              %Calculamos el valor de la portadora a transmitir
